...
export default function GamePostingPage({ onBack }) {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[#e8eff5]">
      <header className="flex items-center justify-between gap-2 bg-brand px-2 py-2 text-white sm:px-3 sm:py-3">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 bg-brand px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))]">
        ...
        <button type="submit" className="shrink-0 rounded-md bg-[#009d20] px-3 text-white sm:px-4">Send</button>
      </form>
    </div>
  )
}
